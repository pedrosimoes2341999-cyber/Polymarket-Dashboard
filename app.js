...
function renderTrack(r){
  ...
  const wallets=new Set(combos.flatMap(c=>(c.wallet_list||[]).map(w=>w.wallet)));
  const entry=combos.reduce((a,c)=>a+Number(c.open_entry||0),0);
  const activityAmount=combos.reduce((a,c)=>a+Number(c.activity_amount||0),0);
  ...
  $('#trackMetrics').innerHTML=
    ...
    `<div class="metric"><span>Entry Cost aberto</span><b>${money(entry)}</b></div>`+
    `<div class="metric"><span>Activity Amount</span><b>${money(activityAmount)}</b></div>`+
  ...
  $('#comboResults').innerHTML=combos.map((c,i)=>{
    const target=(c.legs||[]).filter(l=>
      (r.event?.markets||[]).some(m=>
    ...
    return `<div class="combo">
      <div class="combohead">
        ...
        <b class="positive">${money(c.open_entry)}</b>
        <span>${fmt(c.wallets)} wallets</span>
        <span>${fmt(c.positions)} posições</span>
        <span>${money(c.activity_amount||0)} activity</span>
      </div>

      <div class="combo-grid">
        <div class="combo-section">
          <h3>Seleção no jogo</h3>
          ${target.map(l=>`<div class="pick targetpick">🎯 <b>${esc(l.outcome)}</b> — ${esc(l.market_title||l.event_title)}</div>`).join('')||
            '<div class="pick targetpick">Leg do jogo identificada por condition ID</div>'}
        </div>
        <div class="combo-section">
          <h3>Restantes legs</h3>
          ${other.map(l=>`<div class="pick">• <b>${esc(l.outcome)}</b> — ${esc(l.event_title)} · ${esc(l.market_title)}</div>`).join('')||
            '<div class="pick muted">Sem outras legs</div>'}
        </div>
      </div>
        ...
        <table class="position-table">
          <thead>
            <tr>
              ...
              <th>Shares</th>
              <th>Estado</th>
              <th>1.ª entrada/activity</th>
            </tr>
          </thead>
          <tbody>
            ${(c.wallet_list||[]).map(w=>{
              const activityOnly=w.source==='ACTIVITY';
              const state=activityOnly
                ? '<span class="closed">Sem posição atual</span>'
                : (w.resolved_at
                    ? '<span class="closed">Fechada</span>'
                    : '<span class="open">Aberta</span>');
              return `<tr>
                <td><code class="walletcode">${esc(w.wallet)}</code></td>
                <td><code class="positioncode">${esc(w.position_id||(activityOnly?'ACTIVITY':''))}</code></td>
                <td class="num ${Number(w.entry_cost)>0?'positive':''}"><b>${money(w.entry_cost)}</b></td>
                <td class="num ${Number(w.activity_amount)>0?'positive':''}"><b>${money(w.activity_amount)}</b></td>
                <td class="num">${money(w.activity_payout)}</td>
                <td class="num">${money(w.total_cost)}</td>
                <td class="num">${money(w.current_value)}</td>
                <td class="num">${fmt(w.shares)}</td>
                <td>${state}</td>
                <td>${esc(w.first_entry_at||w.first_activity_at||'')}</td>
              </tr>`;
            }).join('')}
          </tbody>
        </table>
  ...
  }).join('')||'<div class="card">Nenhuma combo encontrada na cobertura indexada.</div>';
